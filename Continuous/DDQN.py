...
class Plan_RL_agent:
    ...
    def limited_expansion(self, node, depth):
        ...
        for a in self.A:
            x_prime, c = self.expandFunc(node.x, a)
            ...
            node.expand(x_prime, self.vFunc(x_prime), a, node.c * c)
    ...
    def train(self, gamma=0.99, max_episodes=400,
        ...
        while training:
            ...
            while done == False:
                ...
                if done:
                    ep += 1
                    self.margin_discrete = 0.5 - pow(0.5, 0.2*ep+1)
                    #self.margin_discrete = 0
                    ...
                    print("\rEpisode {:d} Mean Rewards {:.2f}  Episode reward = {:.2f}  lq = {:.3f}  horizon ={}  ltx ={:3f}  ld ={:3f}  l_spars={:3f}  margin={:3f}\t\t".format(
                        ep, mean_rewards, self.rewards, self.lq, self.horizon, self.ltx, self.ld, self.l_spars, self.margin_discrete), end="")
