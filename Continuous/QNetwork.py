...
from system_conf import Q_HIDDEN_NODES, CODE_SIZE

class Symbolic_Net(nn.Module):
    def __init__(self, n_inputs, n_hidden_nodes, n_outputs, bias, code_size = CODE_SIZE):
        super().__init__()
...
class Net(nn.Module):
    def __init__(self, encoder, symbolicNet):
        super().__init__()
        self.encoder = encoder
        self.symbolicNet = symbolicNet

    def forward(self, data):
        #print(data)
        out = self.encoder(data.to('cuda'))
        out = self.symbolicNet(out).to('cuda')
        #out = self.layers7(out)
        return out
...
class QNetwork(nn.Module):
    ...
    def __init__(self, env, encoder, learning_rate=1e-3,
        ...
        self.symbolic_net = Symbolic_Net(n_inputs, n_hidden_nodes, n_outputs, bias)

        self.network = Net(encoder, self.symbolic_net)
    ...
    def get_enc_value(self,enc_state):
        return self.network.symbolicNet(enc_state)